...
def MCL (particles, control_rtr, distance, LANDMARKS,sig_d=10.0, sig_b=math.radians(8.0), angles_deg=True):
    M = len(particles)
    rot1,trans,rot2 = control_rtr
    for particle in particles:
        x = particle.get_x()
        y = particle.get_y()
        ...
        new_weight = measurement_model(distance (new_x, new_y), LANDMARKS)
        particle.setweight(new_weight)
        
        p = particle.Particle(new_x, new_y, new_theta, )
        particles.append(p)
    ...
    for i in range(M):
        ...
        j = random.choices(range(M), weights=[p.getWeight() for p in particles], k=1)
