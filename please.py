...
def apply_motion_from_cmd(particles, cmd):
    if not cmd:
        return
    kind, val = cmd
    if kind in ("rotate", "rotate_applied"):
...
def execute_cmd(arlo, cmd):
    ...
    if movement == "rotate":
        ...
        time.sleep(0.5)
    elif movement == "rotate_applied":
        # physical rotation already done in motor_control
        time.sleep(0.05)
...
def motor_control(
    ...
    if not hasattr(motor_control, "_last_turn_dir"):
        motor_control._last_turn_dir = +1  # +1 = venstre, -1 = højre
    if not hasattr(motor_control, "_front_hits"):
        motor_control._front_hits = 0  # antal frontblokeringer i træk
