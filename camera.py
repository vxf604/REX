...
class Camera(object):
    ...
    def __init__(self, camidx, robottype="arlo", useCaptureThread=False):
        ...
        if robottype == "arlo":
            ...
            self.distortion_coeffs = np.asarray(
                [0.0, 0.0, 2.0546093607192093e-02, -3.5538453075048249e-03, 0.0],
                dtype=np.float64,
            )
            ...
            self.intrinsic_matrix = np.asarray(
                [
                    500,
                    0.0,
                    3.0757300398967601e02,
                    0.0,
                    500,
                    2.8935674612358201e02,
                    0.0,
                    0.0,
                    1.0,
                ],
            ...
            self.intrinsic_matrix = np.asarray(
                [
                    7.1305391967046853e02,
                    0.0,
                    3.1172820723774367e02,
                    0.0,
                    7.0564929862291285e02,
                    2.5634470978315028e02,
                    0.0,
                    0.0,
                    1.0,
                ],
            ...
            self.distortion_coeffs = np.asarray(
                [
                    1.1911006165076067e-01,
                    -1.0003366233413549e00,
                    1.9287903277399834e-02,
                    -2.3728201444308114e-03,
                    -2.8137265581326476e-01,
                ],
                dtype=np.float64,
            )
            ...
            self.intrinsic_matrix = np.asarray(
                [
                    9.4328095162920715e02,
                    0.0,
                    self.imageSize[0] / 2.0,
                    0.0,
                    9.4946668595979077e02,
                    self.imageSize[1] / 2.0,
                    0.0,
                    0.0,
                    1.0,
            ...
            self.distortion_coeffs = np.asarray(
                [0.0, 0.0, -1.6169374082976234e-02, 8.7657653170062459e-03, 0.0],
                dtype=np.float64,
            )
        else:
        ...
        if piCameraFound:
            ...
            self.cam = picamera.PiCamera(
                camera_num=camidx, resolution=self.imageSize, framerate=30
            )
            ...
            self.picam2_config = self.cam.create_video_configuration(
        ...
        self.arucoMarkerLength = (
            0.15  # [m] actual size of aruco markers (in object coordinate system)
        )
        ...
        if self.useCaptureThread:
            ...
            self.framebuffer = framebuffer.FrameBuffer(
                (self.imageSize[1], self.imageSize[0], 3)
            )
    ...
    def get_next_frame(self):
        ...
        if self.useCaptureThread:
            ...
            if img is None:
                img = np.array(
                    (self.imageSize[0], self.imageSize[1], 3), dtype=np.uint8
                )
    ...
    def detect_aruco_objects(self, img):
        ...
        """
        self.aruco_corners, self.ids, rejectedImgPoints = cv2.aruco.detectMarkers(
            img, self.arucoDict
        )
        self.rvecs, self.tvecs, _objPoints = cv2.aruco.estimatePoseSingleMarkers(
            self.aruco_corners,
            self.arucoMarkerLength,
        ...
        if not isinstance(self.ids, type(None)):
            ...
            for i in range(dists.shape[0]):
                ...
                direction = -1 * np.sign(
                    tobj_xz[0, 0]
                )  # The same as np.sign(np.dot(tobj, xaxis.T))
    ...
    def get_object(self, img):
        ...
        if self.patternFound:
            ...
            if horizontal:
                height = (
                    abs(self.corners[0, 0, 1] - self.corners[2, 0, 1])
                    + abs(self.corners[9, 0, 1] - self.corners[11, 0, 1])
                ) / 2.0
                ...
                height = (
                    abs(self.corners[0, 0, 1] - self.corners[9, 0, 1])
                    + abs(self.corners[2, 0, 1] - self.corners[11, 0, 1])
                ) / 2.0
            ...
            # angle *= -1.0;
            angle = -np.arctan2(
                center - self.intrinsic_matrix[0, 2], self.intrinsic_matrix[0, 0]
            )
            ...
            points = np.array(
                [self.corners[0], self.corners[2], self.corners[9], self.corners[11]]
            )
            ...
            mean_colour = cv2.mean(
                img, mask
            )  # There is a bug here in Python 3 - it produces a segfault
    ...
    def draw_object(self, img):
        ...
        cv2.drawChessboardCorners(
            img, self.patternSize, self.corners, self.patternFound
        )
